...
def baker_hubbard(traj, freq=0.1, exclude_water=True, periodic=True, sidechain_only=False,
                  distance=0.25, angle=2.0 * np.pi / 3.0, pseudo=False):
    """Identify hydrogen bonds based on cutoffs for the Donor-H...Acceptor
    distance and angle.
    ...
    distance_cutoff = distance            # nanometers
    angle_cutoff = angle  # radians
    ...
    # Get the possible donor-hydrogen...acceptor triplets
    bond_triplets = _get_bond_triplets(traj.topology,
        exclude_water=exclude_water, sidechain_only=sidechain_only, pseudo=pseudo)
...
def _get_bond_triplets(topology, exclude_water=True, sidechain_only=False, pseudo=False):
